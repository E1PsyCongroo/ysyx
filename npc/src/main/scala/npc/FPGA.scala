...
class FPGA(awidth: Int, xlen: Int, extentionE: Boolean, PCReset: BigInt, sim: Boolean) extends Module {
  ...
  withClockAndReset(pll.io.clk, !pll.io.locked || reset.asBool) {
    val io    = IO(new FPGAIO)
    val rvcpu = Module(new FPGA_RVCPU(awidth, xlen, extentionE, PCReset, sim))
    val clint = Module(new CLINT(awidth, xlen))
      val bram = Module(new AXI4_RAM_IP)
      val uart = Module(new AXI4_UART_IP)
      AXI4Interconnect(
        Seq(rvcpu.io.master),
        Seq(clint.io, uart.io.axi, bram.io),
        Seq(Dev.fpga.CLINTAddr, Dev.fpga.UARTAddr, Dev.fpga.BRAMAddr),
        Seq(true, true, true)
      )
      uart.io.rx         := io.rx
      io.tx              := uart.io.tx
      rvcpu.io.interrupt := uart.io.interrupt
    rvcpu.io.slave <> AXI4.none
  }
}
