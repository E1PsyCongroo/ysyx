...
class AXI4LiteArbiter(masterNum: Int) extends Module {
  ...
  val writeRequests      = VecInit(io.AXIMasters.map(master => (master.awvalid || master.wvalid))).asUInt
  val readRequests       = VecInit(io.AXIMasters.map(master => master.arvalid)).asUInt
...
class AXI4LiteInterconnect(
  ...
  val waddrRequests      = VecInit(io.AXIMasters.map(master => master.awvalid)).asUInt
  val wdataRequests      = VecInit(io.AXIMasters.map(master => master.wvalid)).asUInt
  val readRequests       = VecInit(io.AXIMasters.map(master => master.arvalid)).asUInt
  ...
  val validSlaves = VecInit(slaves.map(area => area.in(transAddr))).asUInt
  val slaveselect = WireDefault(
    Mux(
