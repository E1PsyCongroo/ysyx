...
class LSU(xlen:Int = 32) extends Module {
  ...
  val devs = Seq(Dev.UART16550Addr, Dev.SPIMasterAddr, Dev.GPIOAddr, Dev.PS2Addr, Dev.VGAAddr)
  SkipDifftest(clock,
    io.out.fire &&
    (devs.map(dev =>
  ...
  SramTracer.io.ren   := io.out.fire && in.ren && (in.raddr >= Dev.SRAMAddr.start.U && in.raddr <= Dev.SRAMAddr.end.U)
  SramTracer.io.waddr := in.waddr
  SramTracer.io.wdata := in.wdata
  SramTracer.io.wlen  := MuxCase(4.U(32.W), Seq(
    memB  -> 1.U(32.W),
    memH  -> 2.U(32.W),
    memW  -> 4.U(32.W),
    memBu -> 1.U(32.W),
