...
class AXILiteArbiter(awidth: Int = 32, dwidth: Int = 32, managerNum: Int) extends Module {
  ...
  val writeRequests       = VecInit(io.AXIManagers.map(
    manager => (manager.awvalid || manager.wvalid)
  )).asUInt
  val readRequests        = VecInit(io.AXIManagers.map(
...
class AXILiteXbar(
  ...
  val waddrRequests       = VecInit(io.AXIManagers.map(
    manager => manager.awvalid
  )).asUInt
  val wdataRequests       = VecInit(io.AXIManagers.map(
    manager => manager.wvalid
  )).asUInt
  val readRequests        = VecInit(io.AXIManagers.map(
    manager => manager.arvalid
  )).asUInt
  ...
  managerSelectReg        := MuxCase(managerSelectReg, Seq(
  ...
  val areas             = VecInit(subordinates.map(area => VecInit(area.start.U, area.end.U)))
