...
#ifdef CONFIG_MTRACE
  if (MTRACE_COND) { log_write(ANSI_FMT("%-16.16s: @" FMT_PADDR ", len = %2d, data = " FMT_WORD "\n", ANSI_FG_CYAN), "read flash", addr, len, data); }
#endif
  return data;
}
...
#ifdef CONFIG_MTRACE
  if (MTRACE_COND) { log_write(ANSI_FMT("%-16.16s: @" FMT_PADDR ", len = %2d, data = " FMT_WORD "\n", ANSI_FG_CYAN), "read psram", addr, len, data); }
#endif
  return data;
}
...
uint8_t* dev_sdram_to_host(paddr_t addr) { return sdram + addr - CONFIG_SDRAM_BASE; }

word_t dev_sdram_read(paddr_t addr, int len) {
  word_t data = host_read(dev_sdram_to_host(addr), len);
#ifdef CONFIG_MTRACE
  if (MTRACE_COND) { log_write(ANSI_FMT("%-16.16s: @" FMT_PADDR ", len = %2d, data = " FMT_WORD "\n", ANSI_FG_CYAN), "read sdram", addr, len, data); }
#endif
  return data;
}
...
#ifdef CONFIG_MTRACE
  if (MTRACE_COND) { log_write(ANSI_FMT("%-16.16s: @" FMT_PADDR ", len = %2d, data = " FMT_WORD "\n", ANSI_FG_CYAN), "write sdram", addr, len, data); }
#endif
...
word_t paddr_read(paddr_t addr, int len) {
  ...
  IFDEF(CONFIG_HAS_SDRAM, if (likely(in_dev_sdram(addr))) return dev_sdram_read(addr, len);)
...
void paddr_write(paddr_t addr, int len, word_t data) {
  ...
  IFDEF(CONFIG_HAS_SDRAM, if (likely(in_dev_sdram(addr))) { dev_sdram_write(addr, len, data); return; })
