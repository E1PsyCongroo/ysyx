...
class ImmGenIO(cpuConfig: CPUConfig) extends Bundle {
  val instruction = Input(UInt(32.W))
  val immType     = Input(UInt(ImmType.getWidth.W))
  val imm         = Output(UInt(cpuConfig.xlen.W))
}

class ImmGen(cpuConfig: CPUConfig) extends Module {
  val io = IO(new ImmGenIO(cpuConfig))
