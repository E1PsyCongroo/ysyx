...
static long load_img() {
  ...
  fseek(fp, 0, SEEK_SET);
  IFDEF(CONFIG_ISA_riscv_ysyxsoc, uint8_t * dev_flash_to_host(paddr_t addr));
  int ret = MUXDEF(CONFIG_ISA_riscv_ysyxsoc,
                   fread(dev_flash_to_host(RESET_VECTOR), size, 1, fp),
...
void init_monitor(int argc, char *argv[]) {
  ...
  /* Initialize devices. */
  IFDEF(CONFIG_DEVICE, init_device(img_file));

  /* Perform ISA dependent initialization. */
  init_isa(wave_file, argc, argv);
